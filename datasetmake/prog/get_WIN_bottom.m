function cell_of_bottoms = get_WIN_bottom(num_person,vec,attribute)
...
[~,LS_array,RS_array,count] = pcount2(vec,attribute);
...
elseif count == 2*num_person - 1
    ...
    if attribute == 0
        [~,LS_array,RS_array,count] = pcount2(vec,1);
        N_RS_array = length(RS_array);
        N_LS_array = length(LS_array);
        if count == 2*num_person
            state = 0;
        ...
        [~,LS_array,RS_array,count] = pcount2(vec,0);
        N_RS_array = length(RS_array);
        N_LS_array = length(LS_array);
        if count == 2*num_person
            state = 0;
